...
#else
	GenerationDistMultiplier = 1.0f;	// temporary
#endif
		...
		// Ped target
			...
			if(CPad::GetPad(0)->CycleCameraModeUpJustDown() && !CReplay::IsPlayingBack() &&
			   ...
			   !m_WideScreenOn && !m_bFailedCullZoneTestPreviously){
				...
				if(FrontEndMenuManager.m_ControlMethod == CONTROL_STANDARD){	// ZAdd: Was fu¢ked up a little. I fixed it. #Camera mode
					if(PedZoomIndicator == CAM_ZOOM_TOPDOWN)
						PedZoomIndicator = CAM_ZOOM_1;
					else if(PedZoomIndicator == CAM_ZOOM_1)	// ZAdd: #Camera mode
						PedZoomIndicator = CAM_ZOOM_2;  // ZAdd: #Camera mode
					else if(PedZoomIndicator == CAM_ZOOM_2) // ZAdd: #Camera mode
						PedZoomIndicator = CAM_ZOOM_3;  // ZAdd: #Camera mode
					else
