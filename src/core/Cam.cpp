...
//	TargetCoors.z += m_fRoadOffSet;
	...
	float LookLeftRight, LookUpDown;
	bool UseMouse = false;
	// ZAdd: Uncommented. Allows to use the mouse while in controller mode. Less jarring. #Input switch
	float MouseX = CPad::GetPad(0)->GetMouseX();
	float MouseY = CPad::GetPad(0)->GetMouseY();
	if((MouseX != 0.0f || MouseY != 0.0f)){	// ZAdd: Removed " && !CPad::GetPad(0)->ArePlayerControlsDisabled()". #Input switch
		UseMouse = true;
		LookLeftRight = -2.5f*MouseX;
		LookUpDown = 4.0f*MouseY;
	}else
