		...
		if (m_nPedState != PED_DIE || bIsPedDieAnimPlaying) {
			...
			switch (m_nPedState) {
				...
				case PED_AIM_GUN:
					...
					} else if(GetWeapon()->IsTypeMelee() && CCamera::m_bUseMouse3rdPerson == true) {	// ZAdd:  Allows stealing money from thieves with keyboard and mouse.	#Steal from thieves kbm
						((CPed*)m_pPointGunAt)->ReactToMeleeThief(this);	// ZAdd:  Allows stealing money from thieves with keyboard and mouse.	#Steal from thieves kbm
						}
					PointGunAt();
					break;
				case PED_SEEK_CAR:
