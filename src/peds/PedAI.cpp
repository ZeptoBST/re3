...
CPed::ReactToPointGun(CEntity *entWithGun)
{
	...
	if (m_pedStats->m_flags & STAT_GUN_PANIC
		&& (m_nPedState != PED_ATTACK || GetWeapon()->IsTypeMelee()) && m_nPedState != PED_FLEE_ENTITY &&
...
void
CPed::ReactToMeleeThief(CEntity *entWithGun)	// ZAdd:  Allows stealing money from thieves with keyboard and mouse.	#Steal from thieves kbm
{
	CPed *pedWithGun = (CPed*)entWithGun;
